...
function setupSheet(sheetName, config) {
  try {
    ...
    // Format date columns
    const dateColumns = [
      SHEET_CONFIG.DATA.COLUMNS.INDEX['PLANNED START DATE'],
      SHEET_CONFIG.DATA.COLUMNS.INDEX['PLANNED END DATE'],
      SHEET_CONFIG.DATA.COLUMNS.INDEX['BUDGET START DATE'],
      SHEET_CONFIG.DATA.COLUMNS.INDEX['BUDGET END DATE'],
      SHEET_CONFIG.DATA.COLUMNS.INDEX['LAST UPDATED']
    ];
